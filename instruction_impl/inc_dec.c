...
void inc_dec_rm_8bit(int mod, int reg, int rm, int scale, int index, int base, unsigned int dis, unsigned int immd)
{
    if (reg == 0)
        inc_or_dec = 0x01;
    else
    ...
    if (mod == 3)
        inc_dec_reg_8bit(mod, reg, rm, scale, index, base, dis, immd);
    else
        inc_dec_mem_8bit(mod, reg, rm, scale, index, base, dis, immd);
}

void inc_dec_reg_8bit(mod, reg, rm, scale, index, base, dis, immd)
{
...
void inc_dec_mem_8bit(mod, reg, rm, scale, index, base, dis, immd)
{
    int w_bit = 0;
...
void inc_dec_rm_32bit(int mod, int reg, int rm, int scale, int index, int base, unsigned int dis, unsigned int immd)
{
    if (reg == 0)
        inc_or_dec = 0x0001;
    else
    ...
    if (mod == 3)
        inc_dec_reg_32bit(mod, reg, rm, scale, index, base, dis, immd);
    else
        inc_dec_mem_32bit(mod, reg, rm, scale, index, base, dis, immd);
}

void inc_dec_reg_32bit(mod, reg, rm, scale, index, base, dis, immd)
{
    printf("INS: Inc_dec reg 8bit\n");
    int w_bit = 1;
    unsigned int op1 = reg_load(rm, w_bit);
...
void inc_dec_mem_32bit(mod, reg, rm, scale, index, base, dis, immd)
{
    int w_bit = 1;
    unsigned int addr = getEffectiveAddressFromModRM(mod, rm, scale, index, base, dis);
    unsigned int op1 = mem_load(addr, w_bit);
